...
struct __msgqueue_node
{
	struct __msgqueue_node *next;
};
...
struct __msgqueue
{
	...
	int linkoff;
	int nonblock;
	struct __msgqueue_node head1;
	struct __msgqueue_node head2;
	struct __msgqueue_node *get_head;
	struct __msgqueue_node *put_head;
	struct __msgqueue_node *put_tail;
...
{
	struct __msgqueue_node *get_head = queue->get_head;
	size_t cnt;
...
{
	struct __msgqueue_node *link;

	link = (struct __msgqueue_node *)((char *)msg + queue->linkoff);
	link->next = NULL;
	pthread_mutex_lock(&queue->put_mutex);
...
void *msgqueue_get(msgqueue_t *queue)
{
	...
	if (queue->get_head->next || __msgqueue_swap(queue) > 0)
	{
		msg = (char *)queue->get_head->next - queue->linkoff;
		queue->get_head->next = queue->get_head->next->next;
	}
	else
		msg = NULL;
		...
		if (ret == 0)
		{
			...
			if (ret == 0)
			{
				...
				if (ret == 0)
				{
					...
					queue->head1.next = NULL;
					queue->head2.next = NULL;
